...
export const CommunityLessonSchema = z.object({
	...
	duracaoSegundos: z
		.number({
			invalid_type_error: "Tipo não válido para a duração da aula em segundos.",
		})
		.int()
		.optional()
		.nullable(),
	...
	muxPlaybackId: z
		.string({
			invalid_type_error: "Tipo não válido para o ID de playback no Mux.",
		})
		.optional()
		.nullable(),
	...
	muxUploadId: z
		.string({
			invalid_type_error: "Tipo não válido para o ID de upload no Mux.",
		})
		.optional()
		.nullable(),
	muxMetadata: CommunityLessonMuxMetadataSchema,
	dataInsercao: z
		.string({ required_error: "Data de inserção nao informada.", invalid_type_error: "Tipo não válido para a data de inserção." })
		.default(new Date().toISOString())
		.transform((val) => (val ? new Date(val) : undefined)),
