					...
					{sale.cliente ? (
						<div className="flex flex-col gap-2">
							<p className="text-sm text-muted-foreground">
								Selecione um endereço cadastrado de {sale.cliente.nome} ou informe um novo.
							</p>
							{/* TODO: ClientLocationSelector component - list client locations */}
							<TextInput
								label="ID do Endereço"
								placeholder="Selecione um endereço..."
								value={checkoutState.state.entregaLocalizacaoId ?? ""}
								handleChange={(value) => checkoutState.setEntregaLocalizacaoId(value || null)}
							/>
						</div>
					) : (
						<p className="text-sm text-muted-foreground">Nenhum cliente vinculado à venda. Volte e adicione um cliente para habilitar endereços de entrega.</p>
					)}
				</div>
			)}
