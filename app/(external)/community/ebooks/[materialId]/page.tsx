...
export default function EbookDetailPage({ params }: { params: Promise<{ materialId: string }> }) {
		...
		return (
			<div className="w-full h-full flex items-center justify-center p-6">
				<div className="text-center">
					...
					<h2 className="text-lg font-semibold mb-2">eBook não encontrado</h2>
					<p className="text-sm text-muted-foreground mb-4">Este conteúdo não está disponível ou não existe.</p>
					<Button variant="outline" asChild size="sm">
						<Link href="/community/ebooks">Voltar para eBooks</Link>
					</Button>
				</div>
			</div>
		);
		...
		<div className="w-full h-full flex flex-col gap-6 p-6">
			<CommunityHeader
				breadcrumbs={[
					...
					{ label: "eBooks", href: "/community/ebooks" },
				...
				<div className="flex-1 flex flex-col gap-3">
					...
					<div className="flex items-center gap-2 mt-2">
						<Button className="rounded-full px-6" onClick={handleDownloadMaterial} disabled={isDownloading || !material.asset?.storageUrl}>
							...
							{isDownloading ? "Baixando..." : "Baixar eBook"}
						</Button>
					</div>
				</div>
			</div>
			...
			<div className="rounded-xl border border-primary/10 bg-card p-4">
				...
				{material.asset?.storageUrl ? (
					<p className="mt-3 text-xs text-muted-foreground flex items-center gap-1.5">
						<FileText className="w-3.5 h-3.5 min-w-3.5 min-h-3.5" />
