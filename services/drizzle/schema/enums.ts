...
export const importJobStateEnum = pgEnum("import_job_state", [
	"AGUARDANDO",
	"EM_ANDAMENTO",
	"PAUSADO_LIMITE",
...
export const onboardingIntentOriginEnum = pgEnum("onboarding_intent_origin", ["LINK", "PARCEIRO", "DEAL", "PERGUNTA", "SEGUNDO_PRODUTO"]);
...
// Um disparo = uma "rodada" de campanha (janela agendada, recorrência ou evento). Os
// destinatários são a fila de trabalho; `interactions` volta a ser só o registro do que foi
// enviado. Enums dedicados em vez de text+$type: o banco recusa valores fora do contrato.
export const campaignDispatchOriginEnum = pgEnum("campaign_dispatch_origin", ["AGENDADA", "RECORRENTE", "EVENTO"]);
...
// entram como novo valor aqui — nunca como um segundo ledger.
export const sendCounterWindowEnum = pgEnum("send_counter_window", ["DIARIO", "SEMANAL"]);
// Estado de entrega de uma interação de campanha. Só existe depois do envio: falhas anteriores ao
