...
test("pagamentos: o troco entra como devolução negativa no fim da célula", () => {
	const summary = buildSalePaymentsSummary([paymentGroup({ metodo: "PIX", valor: 100, valorRecebido: 100 })], [{ metodo: "DINHEIRO", valor: 22 }]);
	assert.equal(summary, "Pix R$ 100,00 (recebido); Troco em dinheiro −R$ 22,00");
});

test("pagamentos: sem troco a célula não ganha sufixo", () => {
	assert.equal(buildSalePaymentsSummary([paymentGroup()], []), "Pix R$ 50,00 (recebido)");
	assert.equal(buildSalePaymentsSummary([paymentGroup()], [{ metodo: "DINHEIRO", valor: 0 }]), "Pix R$ 50,00 (recebido)");
});
...
test("pagamentos: o corte por tamanho nunca come o troco", () => {
	...
	assert.ok(summary.includes("pagamentos)"), summary);
	assert.ok(summary.length <= 200, `comprimento ${summary.length}`);
});
