...
const TARGET_ESTABELECIMENTO_ID = "2952";
...
const FideliziApiResponseSchema = z.object({
	success: z.boolean(),
	message: z.string(),
	data: PaginationDataSchema,
});
type TFideliziApiResponse = z.infer<typeof FideliziApiResponseSchema>;
...
class RateLimiter {
	...
	constructor(requestsPerMinute: number, options: { safetyMargin?: number; maxRequestsPerSecond?: number } = {}) {
		...
		this.refillRate = safeLimit / (60 * 1000);
	...
	async handleRateLimitHit(retryAfterSeconds?: number): Promise<void> {
		this.tokens = 0;
		this.lastRefillTime = Date.now();
		const waitMs = retryAfterSeconds ? retryAfterSeconds * 1000 : 30000;
		console.log(`[RATE-LIMITER] Rate limit hit. Waiting ${waitMs / 1000}s...`);
		await delay(waitMs);
	}
}
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			if (axiosError.response?.status === 429) {
				...
				console.warn(`[FIDELIZI] 429 on ${context} (attempt ${attempt}/${maxRetries})`);
				...
				console.warn(`[FIDELIZI] ${axiosError.response.status} on ${context} (attempt ${attempt}/${maxRetries})`);
				if (attempt < maxRetries) {
...
async function getFideliziClients(client: AxiosInstance, page = 1) {
	...
	return executeWithRateLimitAndRetry(
		async () => {
			const { data } = await client.get<TFideliziApiResponse>(
				`/api/v4/estabelecimentos/${TARGET_ESTABELECIMENTO_ID}/clientes?${searchParams.toString()}`,
			);
			return FideliziApiResponseSchema.parse(data);
		},
		getFideliziClientsRateLimiter(),
		3,
		`clients page ${page}`,
	);
}
...
async function fetchAllFideliziClients() {
	...
	const client = createFideliziClient();

	do {
		const response = await getFideliziClients(client, currentPage);
		...
		console.log(
			`[FIDELIZI] Page ${currentPage}/${totalPages} ` +
				`(${response.data.data.length} clients) | ${elapsed}s elapsed | ~${estimatedRemaining}s remaining | ` +
				`Rate: ${stats.requestsInWindow}/${stats.maxTokens} req/min`,
		);
...
async function writeFideliziClientsToJson(fideliziClients: Awaited<ReturnType<typeof fetchAllFideliziClients>>) {
	const exportDir = path.join(process.cwd(), "tmp");
	await fs.mkdir(exportDir, { recursive: true });
	const filePath = path.join(exportDir, "fidelizi-clients.json");
	await fs.writeFile(filePath, JSON.stringify(fideliziClients, null, 2), "utf-8");
	console.log(`[FIDELIZI] Clients exported to ${filePath}`);
	return filePath;
}

function isRetryableDbError(error: unknown) {
	const err = error as { code?: string; cause?: { code?: string } };
	return err?.code === "ECONNRESET" || err?.cause?.code === "ECONNRESET";
}

async function executeWithDbRetry<T>(fn: () => Promise<T>, context: string, maxRetries = 3, baseDelayMs = 500): Promise<T> {
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			console.warn(`[${context}] DB connection reset, retrying in ${waitMs}ms (attempt ${attempt}/${maxRetries})`);
			await delay(waitMs);
		}
	}
	throw lastError;
...
export async function syncFideliziBalance() {
	if (!process.env.FIDELIZI_APP_TOKEN || !process.env.FIDELIZI_ACCESS_TOKEN) {
		throw new Error("Missing FIDELIZI_APP_TOKEN or FIDELIZI_ACCESS_TOKEN environment variables.");
	}

	const organizationCashbackProgram = await db.query.cashbackPrograms.findFirst({
		where: (fields, { eq: equals }) => equals(fields.organizacaoId, TARGET_ORGANIZATION_ID),
	});
	if (!organizationCashbackProgram) {
		throw new Error("Programa de cashback não encontrado.");
	}
	...
	const ensureCashbackBalanceForClient = async (clientId: string, saldo: number, logKey: string, clientLabel: string) => {
		const existingBalance = await db.query.cashbackProgramBalances.findFirst({
			where: (fields, { and, eq: equals }) =>
				and(
					equals(fields.organizacaoId, TARGET_ORGANIZATION_ID),
					equals(fields.clienteId, clientId),
					equals(fields.programaId, organizationCashbackProgram.id),
				),
		});

		if (existingBalance) {
			console.log(`[${logKey}] Updating cashback balance for ${clientLabel} - ${saldo}`);
			await executeWithDbRetry(
				...
				`${logKey} update cashback balance`,
			);
			return;
		}

		console.log(`[${logKey}] Creating cashback balance for ${clientLabel} - ${saldo}`);
		await executeWithDbRetry(
			() =>
				db.insert(cashbackProgramBalances).values({
					clienteId: clientId,
					programaId: organizationCashbackProgram.id,
					organizacaoId: TARGET_ORGANIZATION_ID,
					saldoValorDisponivel: saldo,
					saldoValorAcumuladoTotal: saldo,
					saldoValorResgatadoTotal: 0,
				}),
			`${logKey} create cashback balance`,
		);
	};

	const clientsInFidelizi = await fetchAllFideliziClients();
	console.log("FIDELIZI CLIENTS COUNT:", clientsInFidelizi.length);
	await writeFideliziClientsToJson(clientsInFidelizi);
	...
	console.log("[SYNC-FIDELIZI-BALANCE] Done:", {
		identifiedClientsCount,
		notIdentifiedClientsCount,
	});
...
void syncFideliziBalance()
	.then(() => {
		process.exit(0);
	})
	.catch((error) => {
		console.error("[SYNC-FIDELIZI-BALANCE] Error:", error);
		process.exit(1);
	});
