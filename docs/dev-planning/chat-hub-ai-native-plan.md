...
> **Status**: implementado nesta branch (Fases 0–4). Decisões tomadas na implementação: retomadas clampadas à janela de 24h (11.1); triagem com Jev **ligada por padrão** em vez de atrás da replay (11.3) — `capacidades.triagem` permite desligar cada gate por organização; modelo econômico e de assistência configuráveis em `modeloConfig`. Pendências operacionais da Fase 2: aplicar `drizzle/0113_ai_agent_follow_ups.sql` (ou `npm run db:push`), o cron `/api/cron/ai-follow-ups` e o trigger de fila `ai-chat-follow-ups` já estão no `vercel.json`; habilitar "Retomadas" por organização em Configurações → Agente de IA. Pendência operacional da Fase 1: adicionar `ampmais_ai_agent_runs` à publicação realtime do Supabase (`alter publication supabase_realtime add table ampmais_ai_agent_runs;`) — sem isso a presença da IA só atualiza no refetch.
