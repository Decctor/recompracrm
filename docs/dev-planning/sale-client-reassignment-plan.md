...
| NF-e (modelo 55) autorizada, em processamento ou com cancelamento pendente                     | **Recusa**                                                                                                      | A CC-e não altera destinatário. A regularização é cancelamento na janela ou devolução + nova emissão. Uma NF-e em nome de X com a venda em nome de Y é exatamente o que uma fiscalização procura.                                                                                  |
| NFC-e autorizada ou em processamento com CPF de X                                              | Permite com confirmação explícita                                                                               | Não há evento de correção para o modelo 65; o CPF é só identificação do consumidor.                                                                                                                                                                                                |
| Documento nunca autorizado (rascunho, pronto, rejeitado, erro), NFC-e sem CPF ou sem documento | Permite sem confirmação                                                                                         | Nunca houve destinatário na SEFAZ: o próximo envio reconstrói o payload a partir da venda e sai em nome de Y. Uma NF-e rejeitada pelo CPF/endereço errado de X é exatamente o caso em que trocar o cliente é a correção. "Cliente pediu CPF na nota" é definir o cliente e emitir. |
...
| Quando Y acumula                                                                               | Agora, se X já acumulava nesta venda ou se a venda está totalmente paga; senão, quando o pagamento se completar | O PDV acumula na confirmação sem esperar pagamento; a loja digital acumula no caminho diferido (`processSaleCashbackAccumulationIfEligible`). A reatribuição espelha o que a venda já fazia.                                                                                       |
...
| Guardas de idempotência do acúmulo                                                             | Ignoram linhas com `metadados.motivoExpiracao = VENDA_REATRIBUIDA`                                              | `accumulateCashbackForClient` e o caminho diferido barravam por (venda, cliente) sem olhar status: num vai-e-volta A → B → A, ou numa venda em aberto reatribuída antes do pagamento, o cashback sumia dos dois lados. Expiração natural e cancelamento continuam bloqueando.      |
...
2. Decide se Y acumula agora (X já acumulava, ou venda paga) — antes da reversão, que expira a linha de X.
3. **Cashback sai**: `transferSaleCashbackAccumulation` (`lib/cashback/transfer-sale-accumulation.ts`) — reversão **restrita ao comprador** (`vendaId` + `clienteId` + `ACÚMULO`), CANCELAMENTO com `motivo: "VENDA_REATRIBUIDA"`, apaga as interações não executadas das campanhas de cashback de X.
4. Atualiza `sales.clienteId`, `saleItems.clienteId` (denormalizado; alimenta produto × cliente) e o título do lançamento contábil.
5. **Cashback entra**: `accumulateCashbackForClient` para Y quando elegível, com `createdAt: dataVenda`.
6. Atribuição: delete em `campaignConversions` da venda + reset das colunas na venda.
7. Log em `rascunhoMetadados.edicoes[]`: `{ tipo: "CLIENTE", clienteAnteriorId, clienteNovoId, cashbackEstornado, cashbackNaoEstornado, cashbackAcumulado, fiscalDesatualizado }`.
...
- Retentativa de documento nunca autorizado após a troca: o provedor Spedy reconstrói o payload do contexto da venda em cada envio (`mapSaleContextToSpedyInvoicePayload`), então rascunho/pronto/rejeitado/erro saem em nome de Y — por isso a política só olha AUTORIZADO, CANCELAMENTO_PENDENTE e EM_PROCESSAMENTO. O `snapshotOrigemVenda` gravado na criação do documento é a fonte da leitura de "CPF na nota".
