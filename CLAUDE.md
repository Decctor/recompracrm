...
- **The frame is `components/ui/section.tsx`, never a new one.** `panel.tsx` holds only what `Section` lacks — the hub's header preset, the list row, the stat tile and the loading/error/empty states — and `Panel` *is* `Section.Root`. This file already went the other way once: it reimplemented border, radius and header, and the single-line header with `truncate` turned into "COMO A ..." on a phone, which is exactly what `Section.Header` (`flex-wrap`) and `Section.Actions` (drops to the next line) solve. `Section` itself exists because two competing shells had to be merged; do not open a third.
- Content goes in `Panel.Body` (with padding) or `Panel.Bleed` (edge to edge: divided lists and tables). `Bleed` cancels `Section.Root`'s padding and `gap-6`, so don't retune the root's gap when you use it.
- Tables use `hub-table.tsx`, which stops being a table below `sm`: the header disappears, the first column becomes the row title and the rest become labelled pairs. A four-column grid does not fit 400px — the uppercase headers collide — and a dashboard glanced at on a phone shouldn't need a horizontal gesture or lose columns.
- Each block owns its query, so one failing endpoint blanks only that block.
