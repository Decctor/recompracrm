...
export default function CloseSalesSession({ sessionId, closeModal, blindCount, callbacks }: CloseSalesSessionProps) {
	...
	const drawerCountedTotal = valorInformado ?? 0;
	const drawerDifference = (Math.round(drawerCountedTotal * 100) - Math.round(drawerExpectedTotal * 100)) / 100;
	const hasDrawerDifference = !blindCount && hasCount && drawerLines.length > 0 && drawerDifference !== 0;
	const sales = session?.vendas ?? [];
	const salesTotal = sales.reduce((acc, sale) => acc + sale.valorTotal, 0);
	const salesBySeller = summarizeSessionSalesBySeller(sales);
	function handleSubmit() {
		if (!session || session.status !== "ABERTA" || !hasCount || isPending) return;
		if (hasDrawerDifference && !confirmarDiferenca) {
			confirmDifference();
			return;
		}
		const counts = drawerLines.map((line) => ({ metodo: line.metodo, valorInformado: Math.round(drawerCountedTotal * 100) / 100 }));
		mutate({ sessaoVendaId: sessionId, conferencias: counts, observacoesFechamento: observacoes || null });
	}
	const actionButtonText = confirmarDiferenca && hasDrawerDifference ? "CONFIRMAR DIFERENÇA E FECHAR" : "FECHAR CAIXA";
	return (
		<ResponsiveMenu
			...
			{!session ? (
				...
				<div className="flex w-full flex-col gap-5">
					...
					<div className="flex flex-col gap-1 rounded-xl bg-muted/50 p-3">
						...
						<SessionMetaRow label="VENDAS DO TURNO" value={`${sales.length} · ${formatToMoney(salesTotal)}`} />
					</div>
					...
					{confirmarDiferenca && hasDrawerDifference ? (
						<div className="flex items-start gap-2 rounded-xl border border-warning/40 bg-warning-surface p-3">
							...
							<p className="text-xs leading-relaxed text-muted-foreground">
								...
								<span className={cn("font-bold tabular-nums", sessionDifferenceClass(drawerDifference))}>{formatSessionDifference(drawerDifference)}</span>.
							...
							<section className="flex flex-col gap-2">
								...
								<div className="flex flex-col overflow-hidden rounded-xl bg-muted/50">
									{drawerLines.map((line, index) => {
										const counted = valorInformado;
										const difference = drawerDifference;
										const showExpected = !blindCount;
										return (
											<div key={line.metodo} className={cn("flex flex-col gap-2 px-3 py-3", index < drawerLines.length - 1 && "border-b border-border/60")}>
												<div className="flex items-center justify-between gap-2">
													<span className="font-bold text-sm">{paymentLabels.get(line.metodo) ?? line.metodo.replaceAll("_", " ")}</span>
													{showExpected ? (
														<span className="text-[11px] text-muted-foreground">
															ESPERADO <span className="font-semibold tabular-nums">{formatToMoney(line.valorEsperado)}</span>
														</span>
													) : (
												...
												{showExpected ? (
													<SessionDrawerComposition
														composicao={{
															...
															troco: line.troco,
															outrasSaidas: line.outrasSaidas,
														}}
														metodo={line.metodo}
														movimentos={session.movimentos}
													/>
												) : null}
												<Field data-invalid={counted !== null && counted < 0}>
													...
													<Input
														...
														disabled={isPending}
														aria-invalid={counted !== null && counted < 0}
												...
												{showExpected && hasCount ? (
													<span className={cn("text-xs font-semibold", sessionDifferenceClass(difference))}>
														{difference === 0
															? "Sem diferença"
															: difference > 0
																? `Sobra ${formatToMoney(difference)}`
																: `Falta ${formatToMoney(Math.abs(difference))}`}
													</span>
												) : null}
											</div>
										);
									...
									<div className="flex flex-col gap-1 border-t border-border/60 bg-muted/80 px-3 py-3">
										{!blindCount ? (
											<div className="flex items-center justify-between text-xs">
												...
												<span className="font-semibold tabular-nums">{formatToMoney(drawerExpectedTotal)}</span>
											</div>
										) : null}
										<div className="flex items-center justify-between text-xs">
											<span className="text-muted-foreground">CONTADO</span>
											<span className="font-semibold tabular-nums">{hasCount ? formatToMoney(drawerCountedTotal) : "Aguardando contagem"}</span>
										</div>
										{!blindCount && hasCount ? (
											<div className="flex items-center justify-between pt-1 text-sm" aria-live="polite">
												<span className="font-bold">DIFERENÇA</span>
												<span className={cn("font-black tabular-nums", sessionDifferenceClass(drawerDifference))}>
													{formatSessionDifference(drawerDifference)}
												</span>
											</div>
										) : null}
									</div>
					...
					{drawerLines.length === 0 && receivableLines.length === 0 ? (
							...
							<CollapsibleTrigger className="group flex w-full items-center justify-between rounded-lg px-1 py-1 text-left transition-colors hover:bg-muted/40">
								<div className="flex flex-col gap-0.5">
									...
									<span className="text-xs text-muted-foreground">
										{sales.length} {sales.length === 1 ? "venda" : "sales"} · {formatToMoney(salesTotal)}
									</span>
								</div>
							...
							<CollapsibleContent>
								<div className="mt-1 flex flex-col gap-1 rounded-xl bg-muted/40 px-3 py-2">
									{sales.map((sale) => (
										...
										>
											<span className="truncate font-medium text-foreground">{sale.cliente?.nome || "AO CONSUMIDOR"}</span>
											<span className="text-right font-semibold tabular-nums">{formatToMoney(sale.valorTotal)}</span>
											<span className="col-span-2 text-[11px] text-muted-foreground">{formatDateAsLocale(sale.dataVenda, true)}</span>
										</div>
									))}
								</div>
							</CollapsibleContent>
