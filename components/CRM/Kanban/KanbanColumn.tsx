	...
	return (
		<div className="flex flex-col w-72 min-w-72 shrink-0">
			<div className="flex items-center gap-2 mb-2 px-1">
				...
				<h3 className="text-sm font-semibold uppercase">{STAGE_LABELS[stage] ?? stage}</h3>
				<span className="text-xs text-muted-foreground ml-auto">{leads.length}</span>
			</div>
			{totalValor > 0 && <div className="text-xs text-muted-foreground px-1 mb-2">{formatToMoney(totalValor)}</div>}
			<div ref={setNodeRef} className={cn("flex flex-col gap-2 p-2 rounded-lg bg-muted/50 min-h-32 flex-1 transition-colors", isOver && "bg-muted")}>
